...
document.getElementById("buttonforfeedback").addEventListener("click", function (event) {
    event.preventDefault();
    const listItems = document.querySelectorAll('#suggestiontable li');

    // Erstellen Sie ein Array aus den Textinhalten der li-Elemente
    const listTexts = Array.from(listItems).map(item => item.textContent);

    // Fügen Sie alle Texte zu einem String zusammen, z.B. durch ein Komma getrennt
    const msg = listTexts.join(', ');
    ...
    set(ref(db, 'feedback/' + new Date().getTime()), {  // Verwenden Sie einen Zeitstempel als Schlüssel
        ...
        document.getElementById("suggestiontable").innerHTML="";
    }).catch((error) => {
